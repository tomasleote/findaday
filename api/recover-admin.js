...
/**
 * Fetches group data for the given groupId from the configured database.
 * @param {string} groupId - The identifier of the group to retrieve.
 * @returns {Object|null} The group data object if found; `null` if the group does not exist or the request fails.
 */
...
/**
 * Update a group's stored admin token hash in the database.
 *
 * @param {string} groupId - ID of the group to update.
 * @param {string} newHash - SHA-256 hex digest of the new admin token to store.
 * @return {boolean} `true` if the update succeeded (HTTP 2xx), `false` otherwise.
 */
...
/**
 * Send the group admin a recovery email containing a new admin link.
 *
 * If the EMAIL_USER or EMAIL_PASSWORD environment variables are not set, the function returns without sending an email.
 *
 * @param {string} adminEmail - The recipient admin email address.
 * @param {string} groupName - The group's display name used in the email subject; falls back to "your group" in the subject if falsy.
 * @param {string} adminLink - The full URL that grants admin access and is included in the email body.
 */
