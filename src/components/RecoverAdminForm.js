...
/**
 * Render a three-tab form that lets an admin recover access via passphrase, email link, or by finding groups with an email.
 *
 * Passphrase: requires Group ID and a recovery passphrase (hashed client-side) and returns an admin token.
 * Email link: requires Group ID and an admin email; a recovery link is emailed and an admin token is returned.
 * Find groups: requires an email and sends a summary of all admin groups to that email (does not return a token).
 *
 * @param {Object} props
 * @param {(groupId: string, adminToken: string) => void} props.onSuccess - Called after a successful passphrase or email recovery with the trimmed Group ID and returned admin token.
 * @param {() => void} props.onCancel - Called when the user cancels the flow.
 * @returns {JSX.Element} The recovery form component.
 */
